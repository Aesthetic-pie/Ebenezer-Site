...
if (servicesStripWrap) {
    ...
    function startAutoScroll() {
        ...
        autoScroll = setInterval(() => {
            ...
            servicesStripWrap.scrollLeft += 1;
   ...
   //Pause on hover
   servicesStripWrap.addEventListener('mouseenter', stopAutoScroll);
   servicesStripWrap.addEventListener('mouseleave', startAutoScroll);
    ...
    servicesStripWrap.addEventListener('mousedown', (e) => {
         ...
         startX = e.pageX - servicesStripWrap.offsetLeft;
         startScrollLeft = servicesStripWrap.scrollLeft;

         servicesStripWrap.style.cursor = 'grabbing';
        
    });

    servicesStripWrap.addEventListener('mousemove', (e) => {
        if (!isDragging) return;

        e.preventDefault();

        const x = e.pageX - servicesStripWrap.offsetLeft;
        const walk = (x - startX) * 1.5;  // Adjust the multiplier for faster/slower dragging
        
        servicesStripWrap.scrollLeft = startScrollLeft - walk;
    });
    ...
    window.addEventListener('mouseup', () => {
        ...
        isDragging = false;
        servicesStripWrap.style.cursor = 'grab';

        setTimeout(startAutoScroll, 1000);
    }, { passive: true });
    
    //Fix after sreen resize
